...
- *최적화 포인트: LLM API 호출(RPD) 제한을 아끼기 위해, 전체 데이터를 한 번에 보내되 LLM은 오직 합격한 "HOT" 딜 10~20건의 결과만 짧은 JSON으로 반환하게끔 프롬프트가 고도화되어 있습니다. 이를 통해 긴 답변으로 인한 토큰 초과 파싱 에러(JSONDecodeError)를 원천 차단합니다.*
