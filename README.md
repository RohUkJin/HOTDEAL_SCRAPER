...
**최종 선별된 "READY" 상태의 데이터가 AI에 의해 분석되며, API Rate Limit 방어를 위해 5개씩 Chunk 단위로 묶어 LLM으로 전송됩니다.**
- *최적화 포인트: LLM API 호출 제한(TPM/RPM)을 아끼기 위해 5건의 데이터를 한 번의 프롬프트로 묶어서 전송합니다. 이때, LLM은 내부 기준을 통과한 진짜 "HOT" 딜만 추려서 구조화된 JSON 양식으로 반환하도록 지시받습니다. 대용량 배치 처리 시 발생하는 429 에러 방지를 위해 재시도 및 동적 대기(Backoff) 로직이 적용되어 안정적인 대량 처리가 가능합니다.*
...
-   **스케줄러**: GitHub Actions를 통해 **매일 06:00 KST**에 자동 실행됩니다. (`cron: '0 21 * * *'`)
